...
report.write_text(json.dumps({'blender':bpy.app.version_string,'meshObjects':len(meshes),'vertices':sum(len(o.data.vertices) for o in meshes),'polygons':sum(len(o.data.polygons) for o in meshes),'blenderBounds':{'min':lo,'max':hi},'units':'meters','sizeBytes':out.stat().st_size},indent=2)+'\n')
