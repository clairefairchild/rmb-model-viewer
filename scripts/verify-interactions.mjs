...
if(walkState.pointerLocked){
 await floorPage.keyboard.press('r');assert.equal((await floorPage.evaluate(()=>window.modelViewer())).shooter.enabled,true);await floorPage.mouse.down({button:'right'});assert.equal((await floorPage.evaluate(()=>window.modelViewer())).shooter.aiming,true);await floorPage.mouse.up({button:'right'});assert.equal((await floorPage.evaluate(()=>window.modelViewer())).shooter.aiming,false);await floorPage.mouse.click(720,450);await floorPage.waitForTimeout(100);assert((await floorPage.evaluate(()=>window.modelViewer())).shooter.effects.projectiles>=0);await floorPage.screenshot({path:new URL('floorplan-cheesy.png',out).pathname});await floorPage.keyboard.press('Escape');await floorPage.waitForFunction(()=>!window.modelViewer().walk);const exited=await floorPage.evaluate(()=>window.modelViewer());assert.equal(exited.shooter.enabled,false);assert.deepEqual(exited.shooter.effects,{projectiles:0,splats:0});
