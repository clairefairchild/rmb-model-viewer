...
try{
 for(const mobile of [false,true]){
  ...
  if(!equipment){
   ...
   if(mobile){const pad=await page.locator('[data-move=forward]').boundingBox(),cdp=await context.newCDPSession(page),x=pad.x+pad.width/2,y=pad.y+pad.height/2;await cdp.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{x,y,id:1}]});await page.waitForFunction(frames=>window.modelViewer().frames>frames+2,preWalk.frames,{timeout:60000});await cdp.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});}
   else{await page.evaluate(()=>dispatchEvent(new KeyboardEvent('keydown',{key:'w'})));await page.waitForFunction(frames=>window.modelViewer().frames>frames+2,preWalk.frames,{timeout:60000});await page.evaluate(()=>dispatchEvent(new KeyboardEvent('keyup',{key:'w'})));}
